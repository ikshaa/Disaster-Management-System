...
import { getPrioritized } from "./services/api";
...
export default function App() {
  ...
  // Derive stats from reports list — always consistent, never double-counts
  const stats = useMemo(() => ({
    total:      reports.length,
    pending:    reports.filter(r => r.status === "pending").length,
    dispatched: reports.filter(r => r.status === "dispatched").length,
    resolved:   reports.filter(r => r.status === "resolved").length,
    critical:   reports.filter(r => r.final_priority >= 8).length,
  }), [reports]);
    ...
    const cleanup = createWebSocket((msg) => {
      ...
      if (msg.type === "new_report") {
        setReports(prev => {
          ...
          if (prev.some(r => r.id === msg.report.id)) return prev;
